...
public class GoHTMXWebSocketBridge: NSObject {
    public static let shared = GoHTMXWebSocketBridge()
    ...
    public func connect(url: String) throws -> String {
        ...
        throw GoHTMXError.unsupported("WebSocket support not yet implemented")
    }
    ...
    func deliverMessage(sessionID: String, data: String) {
        ...
        let js = "window._gohtmx_ws_message('\(sessionID)', '\(escaped)')"
    ...
    func deliverClose(sessionID: String, code: Int, reason: String) {
        ...
        let js = "window._gohtmx_ws_close('\(sessionID)', \(code), '\(reasonEscaped)')"
