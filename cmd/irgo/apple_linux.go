...
// using the xtool Darwin SDK. Mirrors buildIOS (commands.go) but:
...
// copyDir is declared in desktop.go and reused here for the xcframework copy.

// runIOSXtool implements `irgo run ios [--dev]` on non-macOS hosts:
...
func runIOSXtool(devMode bool) error {
	if err := checkTool("xtool", "see https://xtool.sh — then run 'xtool setup'"); err != nil {
		return err
	}

	appDir := filepath.Join("ios", "App")
	if _, err := os.Stat(filepath.Join(appDir, "Package.swift")); err != nil {
		return fmt.Errorf(`SwiftPM iOS app not found at %s

iOS development on Linux uses an xtool (SwiftPM) project at ios/App.
Your scaffold may have been generated with an older irgo version.
Re-run 'irgo new <name>' in a temp dir and copy the ios/App folder over,
or grab the canonical example from github.com/stukennedy/irgo/ios/App`, appDir)
	}
